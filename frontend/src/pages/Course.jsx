          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {courseData.courseContent.map((module) => (
              <div key={module.id} className="p-5 rounded-2xl" style={{ background: '#ffffff', boxShadow: '0 4px 16px rgba(15,15,26,0.06)' }}>
                <span className="text-xs font-bold px-3 py-1 rounded-full inline-block mb-3" style={{ background: 'rgba(255,90,9,0.08)', color: '#FF5A09' }}>{module.module}</span>
                ...
                <ul className="space-y-1.5">
                  {module.topics.slice(0, 4).map((topic, i) => (
                    <li key={i} className="flex items-start gap-2 text-xs sm:text-sm" style={{ color: '#6b7280' }}>
                      <CheckCircle2 className="w-3.5 h-3.5 mt-0.5 flex-shrink-0" style={{ color: '#FF5A09' }} />
                      <span>{topic}</span>
                    </li>
                  ))}
                </ul>
      ...
      <section className="py-16 px-4 relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #000000 0%, #0d0d0d 60%, #000000 100%)' }}>
        ...
        <div className="container mx-auto max-w-5xl relative z-10">
          <h2 className="text-3xl font-bold text-center mb-10" style={{ color: '#ffffff' }}>
            हा course <span style={{ background: 'linear-gradient(135deg, #FF5A09, #FF5A09)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>तुमच्यासाठी</span> आहे का?
          </h2>
